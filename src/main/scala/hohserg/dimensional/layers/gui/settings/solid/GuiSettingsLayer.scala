...
class GuiSettingsLayer(parent: GuiSetupDimensionalLayersPreset, layer: SolidLayerSpec, index: Int)
  ...
  override def initGui(): Unit = {
    ...
    blocksList = addElement(new GuiBlocksList(this, width - 210, height))

    if (currentBlockState != Blocks.AIR.getDefaultState)
      blocksList.select(GuiBlocksList.DrawableBlock(currentBlockState.getBlock))

    heightField = addElement(new GuiNumericField(2, rightPaneMinX + fontRenderer.getStringWidth("height:") + 3, doneButton.y + doneButton.height + 10, 2, layerHeight, layer.height, _.toInt))
