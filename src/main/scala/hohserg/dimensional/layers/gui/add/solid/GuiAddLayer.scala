...
class GuiAddLayer(parent: GuiSetupDimensionLayersPreset) extends GuiBase(parent) with SelectHandler[GuiBlocksList.DrawableBlock] {
  var blocksList: GuiBlocksList = _
  var doneButton: GuiClickableButton = _
  var heightField: GuiTextField = _

  var currentBlockState = Blocks.AIR.getDefaultState
  ...
  override def initGui(): Unit = {
    ...
    doneButton = addButton(new GuiClickableButton(1, width - 100, 10, 90, 20, "Done")(() => {
      back()
      parent.layersList.add(currentBlockState, Try(heightField.getText.toInt).map(clamp(_, 1, 99)).getOrElse(1))
    }
    ) {
      enabled = false
    })
