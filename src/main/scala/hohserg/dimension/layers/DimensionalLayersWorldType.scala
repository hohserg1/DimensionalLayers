...
object DimensionalLayersWorldType {
  lazy val layers: Map[IntRange, World => Layer] =
    Map(
      ...
      IntRange.of(17, 17 + 16) -> (world => FakeWorldLayer(FakeWorld(world.getSeed, enableMapFeatures = true, world => new IceikaChunkGenerator(world, world.getSeed), world => new BiomeProviderIceika(), hasSkyLight = false)))
    )
}
